...
export const EmployeeCard = ({ item, onEdit }: EmployeeCardProps) => {
  // Helper for Initials
      ...
      <View className="flex-row items-center flex-1">
        ...
        <View className="h-12 w-12 rounded-full bg-blue-50 items-center justify-center border border-blue-100 overflow-hidden">
            {item.profileImgUri ? (
               ...
               <Text className="text-blue-600 font-bold text-lg">{getInitials(item.name)}</Text>
            )}
        </View>
        ...
        <View className="ml-4 flex-1">
          <View className="flex-row items-center">
            ...
            <View className={`ml-3 px-2 py-0.5 rounded-full ${statusColor.split(" ")[0]}`}>
              <Text className={`text-xs font-medium ${statusColor.split(" ")[1]}`}>{statusLabel}</Text>
            </View>
          </View>
          <Text className="text-gray-500 text-sm mt-0.5">{item.jobTitle} • {item.department}</Text>
